            ...
            for ( $d = 0, $c = sizeof( $Modules->arr_module_init['sidebar'] ); $d < $c; $d++ ):
                ...
                for ( $_d = 0, $_c = sizeof( $Modules->array_modules[ $Modules->arr_module_init['sidebar'][ $d ] ]['sidebar'] ); $_d < $_c; $_d++ ):?>
                    <li class="tooltip-js" data-tooltip-js="<?php print $d."-".$_d;?>">
                        <a href="<?php echo $General->arr_general['site'] ?><?php echo ($Modules->array_modules[ $Modules->arr_module_init['sidebar'][ $d ] ]['page'] == 'home') ? Modules->array_modules[ $Modules->arr_module_init['sidebar'][ $d ] ]['sidebar'][ $_d ]['href'] : $Modules->array_modules[ $Modules->arr_module_init['sidebar'][ $d ] ]['page'] ?>" <?php $Modules->route == str_replace('/','',$Modules->array_modules[ $Modules->arr_module_init['sidebar'][ $d ] ]['sidebar'][ $_d ]['href']) ? print 'class="table-active"' : false?>
