                ...
                <?php for ( $i = 0, $sz = sizeof( $res ); $i < $sz; $i++ ):
                    ...
                    $res[$i]['aid'] != '0' && $General->get_js_relevance_avatar( $res[ $i ]['admin_authid'] )?><tr>
                        ...
                        <?php if( $General->arr_general['avatars'] != 0 ) {?>
                            <th class="text-right tb-avatar pointer" <?php if ($Modules->array_modules['module_page_profiles']['setting']['status'] == '1'){ ?>onclick="location.href = '<?php echo $General->arr_general['site'] ?>profiles/<?php print $General->arr_general['only_steam_64'] === 1 ? con_steam32to64( $res[ $i ]['authid'] ) : $res[ $i ]['authid']?>' "<?php } ?>><img class="rounded-circle" id="<?php echo con_steam32to64( $res[ $i ]['authid'] ) ?>"<?php echo $i  < '20' ? 'src' : 'data-src'?>="<?php echo $General->getAvatar( con_steam32to64( $res[ $i ]['authid'] ), 2 )?>"></th>
                        <?php } ?>
                        <th class="text-left pointer" <?php if ($Modules->array_modules['module_page_profiles']['setting']['status'] == '1'){ ?>onclick="location.href = '<?php echo $General->arr_general['site'] ?>profiles/<?php print $General->arr_general['only_steam_64'] === 1 ? con_steam32to64( $res[ $i ]['authid'] ) : $res[ $i ]['authid']?>' "<?php } ?>>
                            <a <?php if ($Modules->array_modules['module_page_profiles']['setting']['status'] == '1'){ ?>href="<?php echo $General->arr_general['site'] ?>profiles/<?php print $General->arr_general['only_steam_64'] === 1 ? con_steam32to64( $res[ $i ]['authid'] ) : $res[ $i ]['authid']?>"<?php } ?>><?php echo action_text_clear( action_text_trim($res[ $i ]['name'], 13) )?></a>
                        </th>
                        <?php if( $General->arr_general['avatars'] != 0 ):?>
                            <th class="text-right tb-avatar <?php $res[ $i ]['aid'] != '0' && print 'a-type'?>" <?php if ($Modules->array_modules['module_page_profiles']['setting']['status'] == '1' && $res[ $i ]['aid'] != '0'){ ?>onclick="location.href = '<?php echo $General->arr_general['site'] ?>profiles/<?php print $General->arr_general['only_steam_64'] === 1 ? con_steam32to64( $res[ $i ]['admin_authid'] ) : $res[ $i ]['admin_authid']?>' "<?php } ?>><img class="rounded-circle" id="<?php echo con_steam32to64($res[ $i ]['admin_authid']) ?>"<?php echo $i  < '20' ? 'src' : 'data-src'?>="<?php echo $res[ $i ]['admin_authid'] != 'STEAM_ID_SERVER' ? $General->getAvatar( con_steam32to64( $res[ $i ]['admin_authid'] ), 2 ) : 'storage/cache/img/avatars_random/20.jpg'?>"></th>
                        <?php endif?>
                        <th class="text-left <?php $res[ $i ]['aid'] != '0' && print 'pointer'?>" <?php if ($Modules->array_modules['module_page_profiles']['setting']['status'] == '1' && $res[ $i ]['aid'] != '0'): ?>onclick="location.href = '<?php echo $General->arr_general['site'] ?>profiles/<?php print $General->arr_general['only_steam_64'] === 1 ? con_steam32to64( $res[ $i ]['admin_authid'] ) : $res[ $i ]['admin_authid']?>' "<?php endif; ?>>
                            <a <?php if ($Modules->array_modules['module_page_profiles']['setting']['status'] == '1' && $res[ $i ]['aid'] != '0'): ?>href="<?php echo $General->arr_general['site'] ?>profiles/<?php print $General->arr_general['only_steam_64'] === 1 ? con_steam32to64( $res[ $i ]['admin_authid'] ) : $res[ $i ]['admin_authid']?>"<?php endif; ?>><?php echo action_text_clear( action_text_trim($res[ $i ]['user'], 13) )?></a>
                        </th>
